...
public:
  ...
  void print(){
    ...
    case operandtype::empty: {if (debug>=4) cout<<"emptyop"; break;}
...
void parseline(opcodes::optype & code, operand& op1, operand& op2, ifstream & src, int line) {
  op1.settype(operandtype::empty);
  op2.settype(operandtype::empty);
...
int main(int argc, char *argv[]){
  ...
  while (!done) {
	...
	passert(op2.type()==operandtype::empty,"only one operand expected"); 
	...
	break;
      ...
      case opcodes::push: {
	passert(op2.type()==operandtype::empty,"zero or one operand expected"); 
	passert(op1.type()==num || op1.type()==reg || op1.type()== id
		|| op1.type()== stackref || op1.type()==operandtype::empty ,
		"illegal operand type"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
	...
	break;
      ...
      case opcodes::ret: {
	passert(op1.type()==operandtype::empty && op2.type()==operandtype::empty,"no operand expected"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
      }
      case opcodes::link: {
	passert(op1.type()==num && op2.type()==operandtype::empty,"illegal operand"); 
	program.push_back(opcode(code,op1,op2));
        declarations = false;
	break;
      }
  ...
  // ^^^^^^^^^^^^^^^^^^^^^^^^ Statistics ^^^^^^^^^^^^^^^^^
    ...
    case opcodes::push: {
      if (pc->o1().type() != operandtype::empty) 
