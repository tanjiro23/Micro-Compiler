...
if_stmt : IF {ST->addTable();} '(' cond { back_cnt++; } ')' decl aug_stmt_list {
    WC->lb++;
    WC->lblist.push_front(WC->lb);
    front_cnt++;
    // cerr << "lblist during IF after pushing in front\n";
    // for (int i: WC->lblist) {
    //     cerr << i << " ";
    // }
    // cerr << "\n";
    ...
    WC->lblist.pop_back();
    back_cnt--;
    // cerr << "lblist during IF after popping back\n";
    // for (int i: WC->lblist) {
    //     cerr << i << " ";
    // }
    // cerr << "\n";
...
else_part : ELSE {ST->addTable();} decl aug_stmt_list {ST->pop_table();} |
;
...
for_stmt : FOR {
    ...
    WC->lblist.push_front(WC->lb);
    for_cont--;
    WC->lblistfor.push_back(for_cont);
    // cerr << "lblist during FOR init\n";
    // for (int i: WC->lblist) {
    //     cerr << i << " ";
    // }
    // cerr << "\n";
...
XX : ';' incr_stmt ')' decl aug_stmt_list {
    int x = WC->lblistfor.back();
    WC->lblistfor.pop_back();
    WC->codelines.push_back(new ERline(WC->symbolTable->STstack.top()->scope, "LABEL", "LABEL"+to_string(x)));
}
    ROF { 
    if ($2 != nullptr) {
        ...
        int x = WC->lblist.front();
        WC->lblist.pop_front();
        // cerr << "lblist during ROF after popping front\n";
        // for (int i: WC->lblist) {
        //     cerr << i << " ";
        // }
        // cerr << "\n";
        ...
        x = WC->lblist.back();
        WC->lblist.pop_back();
        // cerr << "lblist during ROF after popping back\n";
        // for (int i: WC->lblist) {
        //     cerr << i << " ";
        // }
        // cerr << "\n";
...
aug_stmt : base_stmt | aug_if_stmt | for_stmt | CONTINUE ';' {
    // cerr << "lblist during continue\n";
    // for (int i: WC->lblist) {
    //     cerr << i << " ";
    // }
    // cerr << "\n";
    // cerr << "lblistfor during continue\n";
    // for (int i: WC->lblistfor) {
    //     cerr << i << " ";
    // }
    // cerr << "\n";
    auto it = WC->lblist.begin();
    ...
    // cerr << "lblist during break\n";
    // for (int i: WC->lblist) {
    //     cerr << i << " ";
    // }
    // cerr << "\n";
    auto it = WC->lblist.end();
    it--;
...
aug_if_stmt : IF {ST->addTable();} '(' cond { back_cnt++; } ')' decl aug_stmt_list {
    WC->lb++;
    WC->lblist.push_front(WC->lb);
    front_cnt++;
    WC->codelines.push_back(new ERline(WC->symbolTable->STstack.top()->scope, "JUMP", "LABEL" + to_string(WC->lb)));
